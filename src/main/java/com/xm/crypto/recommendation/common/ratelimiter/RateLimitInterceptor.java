package com.xm.crypto.recommendation.common.ratelimiter;
...
public class RateLimitInterceptor implements HandlerInterceptor {
    ...
    @Value("${rate-limiter.duration-in-minutes}")
    private long TIME_DURATION;

    // TODO for distributed environment we need to use distributed cache such as Redis cache or similar technologies.
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        if (remoteIpAddress == null || remoteIpAddress.isEmpty()) {
            response.sendError(HttpStatus.BAD_REQUEST.value(), "Missing Header: X-Forwarded-For");
            return false;
        }
